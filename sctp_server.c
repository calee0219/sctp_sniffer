...
int sniffer_server(void) {
    ...
    while (1) {
        ...
        if ((client_data->socket_fd = accept(server_fd, &(client_data->addr),
                                             &client_addr_len)) == -1) {
            perror("accept");
            free(client_data);
            return 7;
        ...
        if (pthread_attr_init(&thread_attr)) {
            printf(
                "Error occureed while initializing thread attributes "
                "structure\n");
            free(client_data);
            return 8;
        }
        ...
        if (pthread_create(&conn_thread, &thread_attr, &handle_connection,
                           (void*)client_data)) {
...
void* handle_connection(void* thread_data) {
    ...
    struct sockaddr_in* p_addr = (struct sockaddr_in*)&client_data
                                     ->addr;  // cast the addr to sockaddr_in
    int socket = client_data->socket_fd;
    char client_ipaddr[128];
    memset(client_ipaddr, 0, sizeof(client_ipaddr));

    if (inet_ntop(AF_INET, &p_addr->sin_addr, client_ipaddr,
                  sizeof(client_ipaddr)) == NULL) {
        perror("inet_ntop");
        return NULL;
    }
